    ...
    chrome.storage.local
      ...
      .then((result) => {
        if (result.authToken && result.userID && result.caption) {
          ...
          fetch(
            `${result.studioDomain}/api/public/v1/media/search?title=${msg.captionDownloadTitle}`,
            {
              method: "GET",
              headers: {
            ...
            res.json().then((data) => {
              fetch(
                `${result.studioDomain}/api/public/v1/media/${data.media[0].id}/caption_files?srclang=en`,
                {
                  method: "POST",
                  headers: {
                    Authorization: `Bearer user_id="${result.userID}",token="${result.authToken}"`,
...
function createExternalMedia(url: string) {
  chrome.storage.local
    .get(["authToken", "userID", "studioDomain"])
    .then((result) => {
      if (result.authToken && result.userID) {
        fetch(
          `${result.studioDomain}/api/media_management/collections/5/media`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer user_id="${result.userID}",token="${result.authToken}"`,
...
function downloadCaption(json) {
  ...
  const getSegments = (events: Array<CaptionEvents>): Segments[] => {
    const segments = [];
    let segmentText = "";
  ...
  const convertToStrTime = (ms: number): string => {
    ...
    return `${(padZero([hours, minutes, seconds]) as Array<string>).join(
      ":"
    )},${padZero([milliseconds], true)[0]}`;
  };

  const padZero = (numList: Array<number>, ms?: boolean) => {
  ...
  const convertSegmentsToSrt = (segments: Segments[]) => {
    let srtString = "";
